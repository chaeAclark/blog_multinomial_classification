...
class AllVsAllModel(Model):
    ...
    def fit_pair(self, x, y, batch_size, epochs, **kwargs):
        ...
        return pair_model.layers[-1].output
    ...
    def predict(self, x):
        ...
        for pair, top_layer in self.pair_layers.items():
            pair_model = Model(self.input, top_layer)
            pair_model.compile(optimizer=self.optimizer, loss=self.loss, metrics=self.metrics)
...
class HierarchicalModel(Model):
    ...
    def fit(self, x, y, epochs, batch_size, epochs_sub=1, batch_size_sub=128, **kwargs):
        ...
        for i in range(len(self.labels)-1):
            ...
            for comb in comb_list:
                ...
                self.comb_layers.update(res)

        #  set combination levels
        self.levels = []
        for i in range(len(self.labels) - 1):
            level = []
            for comb in self.comb_layers.keys():
    ...
    def fit_comb(self, x, y, batch_size, epochs, **kwargs):
        ...
        return comb_model.layers[-1].output
    ...
    def predict(self, x):
        ...
        comb_model = Model(self.input, self.comb_layers[tuple(comb)])
        comb_model.compile(optimizer=self.optimizer, loss=self.loss, metrics=self.metrics)
        ...
        for i, levels in enumerate(self.levels):
            ...
            for col_idx in levels:
                ...
                if row_idx:
                    ...
                    comb_model = Model(self.input, self.comb_layers[tuple(col_idx)])
                    comb_model.compile(optimizer=self.optimizer, loss=self.loss, metrics=self.metrics)
